		...
		<header id="header">
				...
				{	
					...
					<div class="w3-center w3-margin-bottom">
						...
						<input class="w3-input w3-border" type="text" name="P_location" required value= <?php echo $res['P_ref_name'];?>>
					</div>
					Property Type (Flat or House):
						<select class="w3-input w3-border" name="P_type" size="1" required value= <?php echo $res['P_type'];?>>
							<option value="select">Select </option>
							<option value="Flat" >Flat</option>
							<option value="House">House</option> 
						</select><br>
					Property for (Sell or Rent):
						<select class="input" name="P_for_sell_rent" size="1" required value= <?php echo $res['P_for_sell_rent'];?>>
							<option value="select" > Select </option>
							<option value="Sell" >Sell</option>
							<option value="Rent">Rent</option>
					</select><br>
					Size of Property(in square feet):<input type="text" name="P_size" class="input" required value= <?php echo $res['P_size'];?>><br>
					Price per Square Feet:<input type="text" name="P_price_per_unit" class="input" required value= <?php echo $res['P_price_per_unit'];?>><br>
					Owner Name:<input type="text" name="P_owner_name" class="input"  required value= <?php echo $res['P_owner_name'];?>><br>						
						<div class="w3-center w3-margin-bottom">
							<label> Phone Number</label>
							<input class="w3-input w3-border" type="integer" name="P_owner_phone" required value= <?php echo "0"; echo $res['P_owner_phone'];?>>						</div>
						
					Address:<br><input type="text" name="P_owner_address" class="input"  required value= <?php echo $res['P_owner_address'];?>/><br>
					<input type="file" name="image" value="<?php echo '<img src="data:P_pic_1,'.$res['P_pic_1'].'">';?>
					<input type="file" name="image_4" value="<?php echo '<img src="data:P_pic_1,'.$res['P_pic_4'].'">';?>
					<input type="file" name="image_3" value="<?php echo '<img src="data:P_pic_1,'.$res['P_pic_3'].'">';?>
					<input type="hidden"  name="P_approval" value= <?php echo $res['P_owner_phone'];?>/>	
					<input type="hidden"  name="P_ref_id" value ='$_SESSION["U_id"]' >
					<input type="hidden"  name="P_ref_name" value ='$U_name' ></td>
						<button type = "submit" name = "submit" class="btn"> Update Request </button>
				</form>
